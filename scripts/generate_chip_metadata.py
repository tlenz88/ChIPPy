...
This script generates the ChIP-seq metadata file required as input for
the DiffBind R package. The output may be incorrect if replicates do not
share a similar naming convention. This script is meant to be used as a
...
1. input--Directory containing folders with ChIP-seq data. Each sample
   folder should contain a coordinate sorted BAM file and the XLS file
   with called peaks output by MACS.
2. treatment--Name or value indicating the targeted antibody.
3. control--Name or value indicating the contol antibody or input.
"""
